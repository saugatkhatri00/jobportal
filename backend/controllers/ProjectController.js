const{Project,Application,Job} =require('../models/project')
...
const Apply= async(req,res)=>{
    const{email,name,coverletter,salaryexpectation,availability,additionalinfo}=req.body;
    try{
        console.log(1)
        ...
        const createapplication= await Application.create({email,name,coverletter,availability,additionalinfo})//creating new application
        res.status(200).json({createapplication});
    }
    catch(error){
        console.log(7)
        res.status(404).json({error:error.message})
...
const Post= async(req,res)=>{
    const{jobtitle,company,location,salary,jobdescription,requirements,howtoapply}=req.body;
    try{
        console.log(1)
        //Checking if fields are empty
        ...
        const createjob= await Job.create({jobtitle,company,location,salary,jobdescription,requirements,howtoapply})//creating new application
